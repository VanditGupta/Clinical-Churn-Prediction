...
from .schemas import PatientInput, PredictionResponse, ExplanationResponse, BatchPredictionRequest, BatchPredictionResponse
...
async def get_cached_result(cache_key: str, redis_client: Optional[redis.Redis] = None) -> Optional[Dict[str, Any]]:
...
async def set_cached_result(cache_key: str, data: Dict[str, Any], ttl: int = CACHE_TTL, 
...
async def background_prediction_logging(patient_data: Dict[str, Any], prediction_result: Dict[str, Any]):
    """Background task to log predictions for analytics"""
    try:
        # Simulate async logging to database or analytics service
        await asyncio.sleep(0.1)  # Simulate async operation
        print(f"📊 Logged prediction for patient: {patient_data.get('age', 'N/A')} years old")
    except Exception as e:
...
async def startup_event():
    ...
    model, explainer, label_encoders = await loop.run_in_executor(None, load_model_and_explainer)
...
async def root():
    ...
    return {
        ...
        "version": "2.0.0",
        "features": ["async_operations", "caching", "batch_predictions", "background_tasks"],
        "endpoints": {
...
async def predict(patient_input: PatientInput, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if cached_result:
            ...
            background_tasks.add_task(background_prediction_logging, patient_data, cached_result)
        ...
        # Add background task for logging
        background_tasks.add_task(background_prediction_logging, patient_data, response_data)
...
async def explain(patient_input: PatientInput):
    ...
    try:
        ...
        # Cache the result
        await set_cached_result(cache_key, response_data, EXPLANATION_CACHE_TTL, redis_client)
    ...
    try:
        ...
        for patient_data in batch_request.patients:
            task = asyncio.create_task(
                process_single_prediction(patient_data)
            )
            tasks.append(task)
