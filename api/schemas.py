...
class PatientInput(BaseModel):
    ...
    study_type: str = Field(
        ..., description="Study type: Phase I, Phase II, or Phase III"
    )
    condition: str = Field(..., description="Medical condition")
    visit_adherence_rate: float = Field(
        ..., ge=0.3, le=1.0, description="Visit adherence rate (0.3-1.0)"
    )
    tenure_months: float = Field(..., ge=1, le=36, description="Months in study")
    last_visit_gap_days: float = Field(
        ..., ge=0, le=90, description="Days since last visit"
    )
    ...
    contact_frequency: float = Field(
        ..., ge=1, le=8, description="Contact frequency per month"
    )
    support_group_member: bool = Field(..., description="Support group member")
    language_barrier: bool = Field(..., description="Language barrier")
    device_usage_compliance: float = Field(
        ..., ge=0.2, le=1.0, description="Device compliance rate"
    )
    survey_score_avg: float = Field(
        ..., ge=1, le=10, description="Average survey score"
    )
...
class BatchPredictionRequest(BaseModel):
    ...
    max_concurrent: Optional[int] = Field(
        10, description="Maximum concurrent predictions"
    )
...
class CacheStatsResponse(BaseModel):
    ...
    in_memory_cache: Dict[str, int] = Field(
