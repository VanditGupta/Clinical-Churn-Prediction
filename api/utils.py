...
async def preprocess_patient_data_async(patient_data: Dict[str, Any], label_encoders: Dict) -> pd.DataFrame:
    """
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(executor, preprocess_patient_data, patient_data, label_encoders)
...
async def predict_churn_and_clv_async(patient_data: Dict[str, Any], model: lgb.Booster, 
                                     label_encoders: Dict) -> Tuple[float, float]:
    """
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(executor, predict_churn_and_clv, patient_data, model, label_encoders)
...
async def calculate_clv_async(churn_probability: float, monthly_stipend: float, 
                             expected_duration_months: float = None, 
                             discount_rate: float = 0.05, risk_factor: float = 1.0) -> float:
    """
    ...
    return await loop.run_in_executor(
        executor, calculate_clv, churn_probability, monthly_stipend, 
        expected_duration_months, discount_rate, risk_factor
    )
...
async def explain_prediction_async(patient_data: Dict[str, Any], model: lgb.Booster, 
                                  explainer: shap.TreeExplainer, label_encoders: Dict) -> Tuple[Dict[str, float], List[Dict[str, Any]]]:
    """
    ...
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(executor, explain_prediction, patient_data, model, explainer, label_encoders)

async def batch_predict_async(patients_data: List[Dict[str, Any]], model: lgb.Booster, 
                             label_encoders: Dict, max_concurrent: int = 10) -> List[Tuple[float, float]]:
    """
    ...
    async def predict_with_semaphore(patient_data):
        async with semaphore:
            return await predict_churn_and_clv_async(patient_data, model, label_encoders)
...
async def validate_patient_data(patient_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    required_fields = [
        'age', 'gender', 'income', 'location', 'study_type', 'condition',
        'visit_adherence_rate', 'tenure_months', 'last_visit_gap_days',
        'num_medications', 'has_side_effects', 'transport_support',
        'monthly_stipend', 'contact_frequency', 'support_group_member',
        'language_barrier', 'device_usage_compliance', 'survey_score_avg'
    ]
    ...
    if 'visit_adherence_rate' in patient_data and not (0.3 <= patient_data['visit_adherence_rate'] <= 1.0):
