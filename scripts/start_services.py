...
def run_service(service_config, service_key):
    ...
    if service_config['check_method'] != 'redis_ping' and not check_port_available(port):
...
def main():
    ...
    try:
        ...
        if not setup_redis():
            print("⚠️  Redis setup failed. Continuing without Redis (using in-memory cache only)")
        else:
