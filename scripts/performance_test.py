...
async def test_caching_performance(patient_data: Dict[str, Any], num_requests: int = 5) -> Dict[str, List[float]]:
    ...
    return {
        ...
        "cache_hit_improvement": (times[0] - statistics.mean(times[1:])) / times[0] * 100 if times[0] >= 0 and len([t for t in times[1:] if t >= 0]) > 0 else None
    }
...
def print_comparison_results(async_times: List[float], sync_times: List[float], test_name: str):
...
async def main():
    ...
    if batch_time >= 0 and parallel_time >= 0 and sequential_time >= 0:
        ...
        parallel_vs_sequential = (sequential_time - parallel_time) / sequential_time * 100
    ...
    if cache_results["first_request"]:
        ...
        if cache_results["subsequent_requests"]:
            ...
            if cache_results["cache_hit_improvement"]:
                print(f"📊 Cache Hit Improvement: {cache_results['cache_hit_improvement']:.1f}%")
