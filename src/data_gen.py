...
def generate_synthetic_clinical_data(n_records=NUM_RECORDS, random_seed=RANDOM_SEED):
    ...
    conditions = np.random.choice(CONDITIONS, n_records, p=[0.25, 0.2, 0.15, 0.1, 0.1, 0.1, 0.05, 0.05])
    ...
    age_adherence_factor = (85 - ages) / 67 * 0.2  # Younger participants slightly less adherent
    income_adherence_factor = (income - 20000) / 130000 * 0.1  # Higher income slightly more adherent
    visit_adherence_rate = base_adherence + age_adherence_factor + income_adherence_factor
    visit_adherence_rate = np.clip(visit_adherence_rate, VISIT_ADHERENCE_RANGE[0], VISIT_ADHERENCE_RANGE[1])
    ...
    last_visit_gap_days = np.clip(last_visit_gap_days, LAST_VISIT_GAP_RANGE[0], LAST_VISIT_GAP_RANGE[1])
    
    # Number of medications
    num_medications = np.random.poisson(3, n_records)
    num_medications = np.clip(num_medications, NUM_MEDICATIONS_RANGE[0], NUM_MEDICATIONS_RANGE[1])
    
    # Side effects (correlated with number of medications)
    side_effect_prob = 0.3 + (num_medications / 8) * 0.4  # More medications = higher side effect risk
    has_side_effects = np.random.binomial(1, side_effect_prob, n_records)
    
    # Transport support (correlated with location and income)
    transport_prob = 0.2 + (np.array([loc == 'Rural' for loc in locations]) * 0.3) + \
                    (income < 40000) * 0.2
    ...
    study_type_bonus = np.array([0, 100, 200])[np.array([st == 'Phase I' for st in study_types]) * 0 + 
    ...
    monthly_stipend = np.clip(monthly_stipend, MONTHLY_STIPEND_RANGE[0], MONTHLY_STIPEND_RANGE[1])
    
    # Contact frequency (correlated with adherence and tenure)
    base_contact = np.random.poisson(3, n_records)
    adherence_contact_factor = (1 - visit_adherence_rate) * 2  # Lower adherence = more contact
    contact_frequency = base_contact + adherence_contact_factor
    contact_frequency = np.clip(contact_frequency, CONTACT_FREQUENCY_RANGE[0], CONTACT_FREQUENCY_RANGE[1])
    
    # Support group membership (correlated with condition and age)
    support_prob = 0.15 + (np.array([cond in ['Mental Health', 'Cancer'] for cond in conditions]) * 0.2) + \
                  (ages > 60) * 0.1
    support_group_member = np.random.binomial(1, support_prob, n_records)
    
    # Language barrier (correlated with location and income)
    language_prob = 0.1 + (np.array([loc == 'Urban' for loc in locations]) * 0.05) + \
                   (income < 30000) * 0.1
    ...
    age_compliance_factor = (85 - ages) / 67 * 0.3  # Younger participants less compliant
    income_compliance_factor = (income - 20000) / 130000 * 0.2  # Higher income more compliant
    device_usage_compliance = base_compliance + age_compliance_factor + income_compliance_factor
    device_usage_compliance = np.clip(device_usage_compliance, DEVICE_COMPLIANCE_RANGE[0], DEVICE_COMPLIANCE_RANGE[1])
    
    # Survey score (correlated with multiple factors)
    base_score = np.random.normal(7, 1.5, n_records)
    adherence_score_factor = visit_adherence_rate * 2  # Higher adherence = higher satisfaction
    stipend_score_factor = (monthly_stipend - 100) / 900 * 1  # Higher stipend = higher satisfaction
    side_effect_score_factor = has_side_effects * (-1)  # Side effects = lower satisfaction
    survey_score_avg = base_score + adherence_score_factor + stipend_score_factor + side_effect_score_factor
    survey_score_avg = np.clip(survey_score_avg, SURVEY_SCORE_RANGE[0], SURVEY_SCORE_RANGE[1])
    
    # Generate churn target with realistic patterns
    churn_prob = generate_churn_probability(
        visit_adherence_rate, last_visit_gap_days, tenure_months, 
        has_side_effects, survey_score_avg, contact_frequency, 
        device_usage_compliance, monthly_stipend, income
    )
...
def generate_churn_probability(visit_adherence, last_visit_gap, tenure, 
                             has_side_effects, survey_score, contact_freq,
                             device_compliance, stipend, income):
    """
    ...
    satisfaction_effect = ((10 - survey_score) / 10) * 0.25  # Lower satisfaction = higher churn
    contact_effect = (contact_freq / 8) * 0.1  # More contact = slightly higher churn (intervention)
    ...
    income_effect = ((150000 - income) / 130000) * 0.05  # Lower income = slightly higher churn
    
    # Combine all effects
    churn_prob = base_prob + adherence_effect + gap_effect + tenure_effect + \
                side_effect_impact + satisfaction_effect + contact_effect + \
                compliance_effect + stipend_effect + income_effect
