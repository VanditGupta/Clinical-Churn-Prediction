...
def calculate_expected_retention_months(
    churn_probability, base_duration=AVERAGE_STUDY_DURATION_MONTHS
):
    """
...
def generate_predictions_with_clv(data_filepath=None, output_filepath=None):
    ...
    for i, (churn_prob, monthly_stipend) in enumerate(
        zip(churn_probabilities, data["monthly_stipend"])
    ):
...
def print_prediction_summary(predictions_df):
    ...
    print(
        f"Average churn probability: {predictions_df['churn_probability'].mean():.3f}"
    )
    print(f"Predicted churn rate: {predictions_df['predicted_churn'].mean():.3f}")
    print(f"Average CLV: ${predictions_df['clv'].mean():,.2f}")
    print(
        f"Average expected retention: {predictions_df['expected_retention_months'].mean():.1f} months"
    )
    ...
    top_clv = predictions_df.nlargest(5, "clv")[
        ["participant_id", "clv", "churn_probability", "monthly_stipend"]
    ]
    for _, row in top_clv.iterrows():
        print(
            f"  {row['participant_id']}: ${row['clv']:,.2f} (Churn: {row['churn_probability']:.3f}, Stipend: ${row['monthly_stipend']:.0f})"
        )

    print(f"\nBottom 5 CLV Participants:")
    bottom_clv = predictions_df.nsmallest(5, "clv")[
        ["participant_id", "clv", "churn_probability", "monthly_stipend"]
    ]
...
def analyze_clv_by_features(predictions_df):
    ...
    clv_by_study = (
        predictions_df.groupby("study_type")["clv"].agg(["mean", "count"]).round(2)
    )
    print(clv_by_study)
    ...
    clv_by_condition = (
        predictions_df.groupby("condition")["clv"].agg(["mean", "count"]).round(2)
    )
    ...
    clv_by_location = (
        predictions_df.groupby("location")["clv"].agg(["mean", "count"]).round(2)
    )
    ...
    clv_by_risk = (
        predictions_df.groupby("risk_category")["clv"].agg(["mean", "count"]).round(2)
    )
    print(clv_by_risk)
...
def generate_clv_report(predictions_df, output_path=None):
    ...
    with open(output_path, "w") as f:
        ...
        f.write("-" * 15 + "\n")
        risk_analysis = (
            predictions_df.groupby("risk_category")
            .agg({"clv": ["mean", "count", "sum"], "churn_probability": "mean"})
            .round(3)
        )
        ...
        f.write("-" * 20 + "\n")
        study_analysis = (
            predictions_df.groupby("study_type")["clv"]
            .agg(["mean", "count", "sum"])
            .round(2)
        )
        ...
        f.write("-" * 18 + "\n")
        condition_analysis = (
            predictions_df.groupby("condition")["clv"]
            .agg(["mean", "count", "sum"])
            .round(2)
        )
        ...
        f.write("-" * 25 + "\n")
        top_10 = predictions_df.nlargest(10, "clv")[
            ["participant_id", "clv", "churn_probability", "study_type", "condition"]
        ]
        f.write(str(top_10) + "\n\n")

        f.write("Bottom 10 CLV Participants:\n")
        f.write("-" * 28 + "\n")
        bottom_10 = predictions_df.nsmallest(10, "clv")[
            ["participant_id", "clv", "churn_probability", "study_type", "condition"]
        ]
