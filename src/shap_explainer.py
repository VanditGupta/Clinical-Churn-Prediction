...
class SHAPExplainer:
    ...
    def explain_prediction(self, patient_data: Dict[str, Any]) -> Tuple[Dict[str, float], List[Dict[str, Any]]]:
        """
    ...
    def explain_batch(self, patient_data_list: List[Dict[str, Any]]) -> List[Tuple[Dict[str, float], List[Dict[str, Any]]]]:
        """
...
def create_shap_explainer(model_path: str = None, metadata_path: str = None) -> SHAPExplainer:
    """
...
if __name__ == "__main__":
    ...
    for i, feature_info in enumerate(feature_importance[:5]):
        print(f"  {i+1}. {feature_info['feature']}: {feature_info['shap_value']:.4f} ({feature_info['direction']})") 