...
def plot_feature_importance(model, feature_names, top_n=15):
    ...
    plt.title(f'Top {top_n} Feature Importance for Churn Prediction', fontsize=14, fontweight='bold')
...
def plot_roc_curve(model, X_test, y_test):
    ...
    plt.plot(fpr, tpr, color='#2E86AB', lw=3, label=f'ROC curve (AUC = {auc_score:.3f})')
    plt.plot([0, 1], [0, 1], color='#A23B72', lw=2, linestyle='--', label='Random Classifier')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
...
def plot_prediction_distribution(y_test, y_pred_proba):
    ...
    for label in [0, 1]:
        mask = y_test == label
        ax1.hist(y_pred_proba[mask], bins=30, alpha=0.7, 
                label=f'Actual {"Churn" if label else "No Churn"}', 
                color='red' if label else 'blue')
    
    ax1.set_xlabel('Predicted Probability', fontsize=12)
    ax1.set_ylabel('Frequency', fontsize=12)
    ax1.set_title('Prediction Probability Distribution by Actual Class', fontsize=14, fontweight='bold')
    ax1.legend()
    ...
    ax2.set_title('Prediction Probability Distribution (Box Plot)', fontsize=14, fontweight='bold')
...
def generate_shap_analysis(model, X_train, X_test, feature_names):
    ...
    top_features = ['last_visit_gap_days', 'visit_adherence_rate', 'tenure_months', 
                   'monthly_stipend', 'survey_score_avg']
    
    for i, feature in enumerate(top_features):
        if feature in X_test.columns:
            plt.figure(figsize=(10, 8))
            # Create dependence plot using the correct API
            shap.plots.scatter(shap_explanation[:, feature], 
                              show=False)
    ...
    for i, idx in enumerate(sample_indices):
        if idx < len(X_test):
            ...
            plt.title(f'SHAP Waterfall Plot - Sample Prediction {i+1}', 
                     fontsize=14, fontweight='bold')
            plt.tight_layout()
    ...
    for i, idx in enumerate(high_risk_indices):
        ...
        plt.title(f'SHAP Force Plot - High-Risk Case {i+1} (Churn Prob: {y_pred_proba[idx]:.3f})', 
                 fontsize=14, fontweight='bold')
        plt.tight_layout()
...
def main():
    ...
    # Start MLflow run
    with mlflow.start_run(run_name=f"clinical_churn_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        ...
        X_train, X_test, y_train, y_test, label_encoders = load_and_preprocess_data(RAW_DATA_FILE)
        ...
        explainer, shap_values = generate_shap_analysis(model, X_train, X_test, X_train.columns)
    ...
    print(f"  - Prediction distribution: {VISUALIZATIONS_DIR}/prediction_distribution.png")
